...
/*   Updated: 2016/12/22 17:21:43 by kdavis           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		fill_row(int fd, int w, t_node *row)
{
	...
	while (++i < w)
	{
		if ((ern = get_int(&tail, &(row + i)->z)) < 1)
			return (fdf_freer(ern, (void*)&head, 0));
		if (*tail == ',')
		{
			tail++;
...
static t_node	**fill_map(int fd, int h, int w)
{
	...
	i = 0;
	if (!(map = (t_node **)ft_memalloc(sizeof(t_node *) * h)))
		return (NULL);
	while (i < h)
	{
		if (!(map[i] = (t_node *)ft_memalloc(sizeof(t_node) * w)) ||
				(fill_row(fd, w, map[i])) < 1)
		{
			ft_delgrid((void***)&map);
			return (NULL);
		}
